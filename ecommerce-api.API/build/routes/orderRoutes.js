...
router.patch('/order/:id/status', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
        const updatedOrder = await order_1.Order.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true });
        if (!updatedOrder)
...
router.put('/order/:id', authMiddleware_1.authMiddleware, async (req, res) => {
    try {
...
router.get("/admin/orders", authMiddleware_1.authMiddleware, async (req, res) => {
    try {
